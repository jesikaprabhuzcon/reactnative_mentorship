...
const Project = (props) => {
  ...
  const getData = async () => {
    ...
    if (projectFilter != "") {
      ...
      const projectInfo = await postGetAllProject(projObj);
      console.log("project filter", projectInfo.data.errors);
      if (projectInfo.data.errors != "") {
        alert(projectInfo.data.errors);
        setProjectRecords(projectInfo.data.data);
      } else {
        setProjectRecords(projectInfo.data.data.projects);
      }
      ...
      const projectInfo = await postGetAllProject(projObj);
      console.log("prj info", projectInfo.data.data.projects);
      setProjectRecords(projectInfo.data.data.projects);
    }
  };
